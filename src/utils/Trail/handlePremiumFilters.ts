...
export const handlePremiumFilters = async (formData: any, size: number, user: UserCookie, token: string | undefined) => {
  if (formData.get('kindness') !== '1') {
    ...
    const resRatingKindness = await fetch('gametrail-backend-s4-production.up.railway.app/api/createMinRating', {
      method: 'POST',
  ...
  if (formData.get('funny') !== '1') {
    ...
    const resRatingFunny = await fetch('gametrail-backend-s4-production.up.railway.app/api/createMinRating', {
      method: 'POST',
  ...
  if (formData.get('teamwork') !== '1') {
    ...
    const resRatingTeamwork = await fetch('gametrail-backend-s4-production.up.railway.app/api/createMinRating', {
      method: 'POST',
  ...
  if (formData.get('ability') !== '1') {
    ...
    const resRatingAbility = await fetch('gametrail-backend-s4-production.up.railway.app/api/createMinRating', {
      method: 'POST',
  ...
  if (formData.get('availability') !== '1') {
    ...
    const resRatingAvailability = await fetch('gametrail-backend-s4-production.up.railway.app/api/createMinRating', {
      method: 'POST',
